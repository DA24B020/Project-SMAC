...
# 1. Load a ≥1 MP image (change the path to your image file)
img = cv2.imread('why.jpg', cv2.IMREAD_GRAYSCALE)
if img is None:
    raise FileNotFoundError("Put a valid image at 'why.jpg'")

# 2. Define two 3×3 kernels:
#    (i) Sharpening
K_sharp = np.array([[ 0, -1,  0],
                    [-1,  5, -1],
                    [ 0, -1,  0]], dtype=np.float32)

#    (ii) Sobel‑style horizontal edge detector
K_edge  = np.array([[-1, -2, -1],
                    [ 0,  0,  0],
                    [ 1,  2,  1]], dtype=np.float32)

# 3. Apply convolution (zero‑pad boundaries) via OpenCV
...
# 4. Plot original and filtered images
...
# 5. Plot histograms before/after
