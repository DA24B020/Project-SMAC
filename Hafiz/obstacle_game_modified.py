...
def get_a_star_path(start, goal, obstacles, width, height, step=10):
    ...
    start_node = (int(start[0] // step * step), int(start[1] // step * step))
    goal_node = (int(goal[0] // step * step), int(goal[1] // step * step))
    
    # open_set: (priority, current_node)
    ...
    while open_set:
        ...
        if heuristic(current, goal_node) < step:
            # Reconstruct path
            path = []
        ...
        for dx, dy in [(0,step),(step,0),(0,-step),(-step,0),(step,step),(-step,-step),(step,-step),(-step,step)]:
            ...
            if not (0 <= neighbor[0] < width and 0 <= neighbor[1] < height):
                continue
            if point_in_any_obstacle(neighbor[0], neighbor[1], obstacles, margin=5):
                continue
    ...
    while running:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if event.key == pygame.K_SPACE:
                        if start_pos and goal_pos:
                            ...
                            a_star_path = get_a_star_path(start_pos, goal_pos, obstacles, WIDTH, HEIGHT)
        ...
        if game_state == "solving":
            ...
            if a_star_path and len(a_star_path) > 1:
                pygame.draw.lines(screen, (0, 0, 0), False, a_star_path, 5)
            if start_pos:
